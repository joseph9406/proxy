module ::Proxy::PluginValidators
  class Base
    ...
    def required_setting?
      @plugin.plugin_default_settings.has_key?(@setting_name)  # has_key? 是 Hash 类的方法之一，用于检查哈希表中是否包含指定的键。
    end
  ...
  class Presence < Base
    def validate!(settings)
      ...
      empty_value = setting_value.nil?  # if setting_value == nil then return true else return false
      empty_value ||= setting_value.empty? if setting_value.is_a?(String)  # 先判斷 if
